...
extern "C" {
    ...
    */
    ...
    // Per-split execution timing, used by ATSInfer to obtain the measured t_c / t_g the
    // placement and dynamic-transfer solvers need (see arXiv 2607.10183v2 section 4.3.1).
    //
    // Backends compute asynchronously, so wall-clock around a split would only measure
    // launch overhead. While profiling is enabled the scheduler therefore synchronizes the
    // split's backend after each split, which makes the timings exact but serializes the
    // round: enable it for occasional calibration rounds, never for steady-state decode.
